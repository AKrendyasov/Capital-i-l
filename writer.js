...
function dbinputer(company,houseList,usedYears,usedMonths,res){
            ...
            usedYears.forEach(function(item, i, arr) {
                usedMonths.forEach(function(mItem, mI, mArr) {
                    worksheet.eachRow(function (row, rowNumber) {
                        if(item==row.values[3]&&mItem==row.values[4]&&company==row.values[1]&&houseList[0]==row.values[2]){
                            var correctionExists=false;
                            var primaryData=row.values;
                            console.log("Cовпадение найдено по адресу " + houseList[0]);
/*                            var totalData=row.values;*/
/*                            worksheet2.eachRow(function (row, rowNumber) {

                                if (primaryData[3] == row.values[3] && primaryData[4] == row.values[4] && primaryData[1] == row.values[1]&& primaryData[2] == row.values[2]) {
                                    console.log(primaryData[3])
                                    console.log(row.values[3])

                                    correctionExists=true;
                                    console.log("Для первички найдена корректировка")
                                    totalData=[];
                                    for (i = 1; i < 21; i++) {
                                        if(i<6){
                                            totalData.push(primaryData[i]);
                                            console.log("Значение некорректируемых данных " + primaryData[i])
                                        }else{
                                          /!*  push(primaryData[i]+row.values[i])*!/
                                            totalData.push(Number(primaryData[i]))
                                            console.log("Значение корректируемых данных "+ Number(primaryData[i]))
                                        }

                                    }

/!*                                    var house = new House(totalData,houseList[0]);
                                    bdArray.push(house);*!/
                                ...
                                var house = new House(primaryData,houseList[0]);
                                bdArray.push(house);
